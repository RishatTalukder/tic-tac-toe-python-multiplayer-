...
def waiting_for_connection():
    ...
    (
        conn,
        addr,
    ) = sock.accept()  # it will wait for a connection , also blocks any new threads
                ...
                if pygame.mouse.get_pressed()[0]:
                    if turn and not (grid.game_over):
                        ...
                        Cell_X, Cell_Y = (
                            pos[0] // 200,
                            pos[1] // 200,
                        )  # this gets me the cell being clicked
                        ...
                        if grid.get_cell_value(Cell_X, Cell_Y) == 0:
                            ...
                            # server sending data
                            send_data = "{}-{}-{}-{}-{}".format(
                                Cell_X, Cell_Y, "yourturn", playing, str(grid.winner)
                            ).encode()
                            conn.send(send_data)
